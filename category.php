							...
							<div class="col-md-12 align-self-center p-static order-2 text-center">
								...
								<h1 class="text-dark font-weight-bold text-8">
                                    <?php
                                        if(isset($_GET['id'])){
                                            $categoryId = $_GET['id'];
                                            $sql        = "SELECT name AS 'catName' FROM category WHERE id = '$categoryId'";
											$cdb         = mysqli_query($db, $sql);
                                            
                                            while( $row = mysqli_fetch_assoc($cdb) ){
                                                extract($row);
                                                echo $catName;
                                            }
							...
							<div class="blog-posts">
								...
								<?php
                                    ...
                                    if(isset($_GET['id'])){
                                        ...
                                        $catId       = $_GET['id'];
                                        $catPost     = "SELECT * FROM post WHERE category_id = '$catId' AND status = 1 ORDER BY id DESC";
								        $postDb      = mysqli_query($db, $catPost);
                                        $numberofRow = mysqli_num_rows($postDb);

                                        if( $numberofRow == 0 ){
                                            echo "<div class='alert alert-info'>Opps!!  No Post Found in this Category</div>";
                                        }
                                        else {
